...
BMP085::reading BMP085::getBoth () { // Get uncompensated pressure.
	int ucPress;
...
// Calculate true pressure.
	...
	x1 >>= 16;
	x2 = (-7357 * p) >> 16;
	p += (x1 + x2 + 3791) >> 4; // Return struct with temperature.
	BMP085::reading data = {
		getCelcius(b5),
		(float)p / 1000.0f
	};
...
int BMP085::getRelativeAltitude (float pressure) { // This formula is from sec. 3.6 of the data sheet.
...
/* Internal */ float BMP085::getCelcius (long b5) {
	long t = (b5 + 8) >> 4;
...
BMP085::smbusIOException::smbusIOException (const char* m, int err) :
	err(err), msg(m) { } const char* BMP085::smbusIOException::what () throw () {
	string m = msg;
	if (err) {
		m += ":\n";
		m += strerror(errno);
